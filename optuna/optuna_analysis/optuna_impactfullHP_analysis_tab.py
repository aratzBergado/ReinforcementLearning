...
def objective(trial):
    ...
    lr_episodes_decay = trial.suggest_categorical("lr_episodes_decay", [100, 1000, 10_000])
    discount_rate = trial.suggest_float("discount_rate", 0.95, 1.0, step=0.01)
    epsilon = trial.suggest_float("epsilon", 0.0, 0.5, step=0.05)
    n_steps = 50_000
    
    env = DiscretizedObservationWrapper(FlappyEnv(render_mode=None), n_bins=20)
    ...
    algo.learn(epsilon_policy, n_steps=n_steps, discount_rate=discount_rate,
               lr=lr, lrdecay=lr_decay, n_episodes_decay=lr_episodes_decay)
    
    avg_reward = evaluate_tabular(env, algo.q_table, max_policy, n_episodes=5)
    env.close()
    return avg_reward
...
if __name__ == "__main__":
    seed = 42
    random.seed(seed)
    os.makedirs("./optuna/FlappyPPO/", exist_ok=True)
    study = optuna.create_study(direction='maximize', study_name="FlappyTabular", storage="sqlite:///optuna/flappy_tabular.db", load_if_exists=True)
    study.optimize(objective, n_trials=100)

    with open("optuna/FlappyTabular/best_trial.json", "w") as f:
        json.dump(study.best_trial.params, f, indent=4)

from optuna.visualization import plot_optimization_history, plot_param_importances, plot_slice, plot_parallel_coordinate, plot_contour

study = optuna.load_study(study_name="FlappyTabular", storage="sqlite:///optuna/flappy_tabular.db")

full_study_dir_path = "optuna/FlappyTabular"
